...
class sell(commands.Cog):
    """
   Envoyons-nous une vente dans le canal désigné.
    """
            ...
            async with ctx.channel.typing():
                ...
                if config is None:
                    ...
                    sellion_channel = self.bot.get_channel(
                        int(config["sellion-channel"]["channel"])
                    )
    ...
    async def setsellchannel(self, ctx, channel: discord.TextChannel):
        ...
        **Usage**:
        ...
        await self.coll.find_one_and_update(
            {"_id": "config"},
            {"$set": {"sellion-channel": {"channel": str(channel.id)}}},
            upsert=True,
        )
        embed = discord.Embed(
    ...
    async def sellchannel(self, ctx):
        """Displays the sellion channel."""
        config = await self.coll.find_one({"_id": "config"})
        sellion_channel = self.bot.get_channel(
            int(config["sellion-channel"]["channel"])
        )
        embed = discord.Embed(
            title=f"The sellion channel is: #{sellion_channel}",
            description="To change it, use [p]setsellchannel.",
            color=0x4DFF73,
        )
        await ctx.send(embed=embed)
    ...
    async def block(self, ctx, user: discord.User, *, reason="Reason not specified."):
        """
        Block a user from using the sell command.
        **Examples:**
        [p]sellmod block @RealCyGuy for abuse!
        [p]sellmod ban 543225108135673877 `cause he's the same person!!!
        """
    ...
    async def unblock(self, ctx, user: discord.User):
        """
        Unblock a user from using the sell command.
        **Examples:**
        [p]sellmod unblock @RealCyGuy
        [p]sellmod unban 543225108135673877
        """
